...
export default function EditMoviePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const addSlot = () => {
        ...
        const newStart = lastSlot ? lastSlot.start : '18:00';
        const newEnd = lastSlot ? lastSlot.end : '';
        setScheduleForm(prev => ({
            ...prev,
            slots: [...prev.slots, { start: newStart, end: newEnd }]
        }));
    };
    ...
    const addShowtime = () => {
        ...
        scheduleForm.slots.forEach(slot => {
            ...
            // Check existing showtimes AND showtimes added in this loop
            const isDuplicate = newShowtimes.some(st =>
                                        ...
                                        <div className="grid grid-cols-1 gap-2">
                                            {times
                                                ...
                                                .map((st) => {
                                                    ...
                                                    return (
                                                        <div key={st.id || `${st.start_time}-${st.hall_id}-${originalIndex}`} className="flex items-center justify-between bg-white/5 p-3 rounded-lg border border-white/5">
                                                            <div className="flex items-center gap-4">
